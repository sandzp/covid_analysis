...
```{r fitted_v_residuals, message=FALSE, echo = TRUE}
...
```{r residuals_v_explanatory, message=FALSE, echo=TRUE}
...
```{r colinearity_assumptions, echo=TRUE, warning = FALSE}
std_errors = list(
  sqrt(diag(vcovHC(model_1))),
  sqrt(diag(vcovHC(model_2))),
  sqrt(diag(vcovHC(model_3)))
...
```{r scale_location, echo=TRUE, warning=FALSE}
plot_1_sl <- model_1 %>%  
...
```{r qq_histograms, message=FALSE, warning=FALSE, echo = TRUE}

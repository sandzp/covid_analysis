...
Some of the data was missing from the initial table and was manually populated on 10/30/2020 using this (website)[https://www.nbcnews.com/health/health-news/here-are-stay-home-orders-across-country-n1168736]. The table below shows the data populated manually (empty cells were left as is):
...
=======
The summary table of the numeric variables show that there are no obvious errors to the variables used in the initial model. Minimums and maximums of case_rate and test_rate are all within the range of possibilities. The case_rate is never greater than test_rate, and the similarity between mean and median means there is not a heavy skew left or right. The heatmap after shows that there are no missing values either.
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
...
=======
The case_rate, our main dependent variable, has a minimum of 344, maximum of 4489, median of 2633, and mean of 2749. From the histogram below, we show the density plot as both a histogram and continuous plot. The density curve has a large standard deviation that is in part due to the limited amount of samples. Our sample set only contains 51 datapoints. With additional datapoints, the true curve may look different.
...
Our first regression model has **Covid-19 Case Rate per 100,000 habitants** as the outcome variable and two covariates: our variable of interest (**Mandatory Mask Use**) and **Test Rate per 100,000 habitants**.  
...
=======
The age groups at the most extremes, below 25 and above 55, have the highest correlation with case rate. We explore the effects of combining the age groups, 0-18 and 19-25 in one group, and 55-64 and 65+ in the other. From the models below, we see that the younger age group maintains the high correlation that was seen in the pairs plot above. Furthermore, by combining the younger age groups, the high correlation is maintained and we are able to better capture the affect of age on the case_rate, while avoiding the effects of collinearity between the two age groups if they were both added to the model.
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
...
=======
df_age$age_above_55 = df$age_0_18 + df$age_19_25
df$age_above_55 = df$age_55_64 + df$age_65

below_25 <- lm(case_rate ~ age_below_25, data = df)
above_55 <- lm(case_rate ~ age_above_55, data = df)

summary(below_25)$r.squared
summary(above_55)$r.squared
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
```
...
=======
We explore the bivariate relationship between case_rate and black_pop below. The untransformed relationship shows a tapering off in case_rate as the percentage of black population increases.
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
...
According to Model 2, suggests that enforcing a mandatory mask use policy would expect to reduce the COVID-19 positive case rate by ~-919.2 covid cases per 100,000 population. Given that the median case rate across US states is 2,633 per 100,000 population, the coefficient estimate has practical significance, with an effect size corresponding to a reduction of 34.9% of the median State case rate value.
...
=======
From the pairs plot below, we see a -0.241 correlation between the number of days that businesses were closed and the case rate. We also see that the number of days that states mandated shelter-in-place is even more strongly correlated at -0.327. For model 3, our goal is not to necessarily improve the model, rather we are looking to stress test it by adding other correlating variables to see the change in impact to our main independent variable, mask_use. 
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
...
=======
The addition of the two days sheltered in place, and days businesses closed leads to our final causal diagram below.
>>>>>>> 5390c0b264bdda68f4a09d8f1edb3e4d6f7c46fe:second_draft.Rmd
...
According to Model 3, states that have adopted mandatory mask use would expect to have 913.8 fewer COVID cases per 100,000 population. Given that the median case rate among US states is 2,633 per 100,000 populations, the coefficient estimate has practical significance, with an effect size corresponding to a reduction of 34.7% of the median rate.

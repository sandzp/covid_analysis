...
<center>*Does the implementation of a mandatory face mask policy aid in reducing the case rate of COVID-19 in the United States?*</center>
...
Below is the causal diagram that our model is built from. In blue is case rate per 100k, the main dependent variable. We hypothesize that policies for mandatory mask (green) has a causal effect on the case rate per 100k. In yellow are the control variables that we use. Finally, in red we have all the other terms combined as a single error term.

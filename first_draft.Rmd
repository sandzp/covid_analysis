...
We find that the highest correlation of the mobility variables lies with the change in Transit Stations (mob_TS). Oddly enough, Wyoming is one of three states with a positive change in transit station use. Wyoming also has by far the largest change at 21% with South Dakota next at 3%. 
...
# variable transformations needed for model building
# may be DELETED after same transformations were implemented in the EDA section
df$age_below_25 = df$age_0_18 + df$age_19_25
df$black_pop[df$black_pop == 0] = 0
```
...
df_mod2 <- merge(df_race, df_socio, by = c("state", "case_rate")) %>% 
  merge(., df_dist, by = c("state", "case_rate")) %>% 
  merge(., df_age, by = c("state", "case_rate"))
```
...
Next we look towards the socio-economic factors. From the socioeconomic factorsr, we see that poverty rate and household income have the highest absolute correlation with case rate per 100k. They are comparable to the correlation of black_pop variable. The black_pop variable also has high collinearity with household_income, poverty_rate, life_expectancy, and unemployment_rate. The percent black population variable may act as a good variable that can control for many of these factors.
...
```{r message=FALSE}
df_race$black_pop[df$black_pop == 0] = 0.01
df_race %>%
  ggplot(aes(y = case_rate, x = black_pop)) +
  geom_point() +
  geom_smooth(method = "lm", level = 0)
  labs(
    title = "Percentage Black in Total Population and Case Rate by state",
    x = "% of Population: Black",
    y = "Case Rate per 100K"
  )
...
As such, we have selected 5 hypothetical omitted variables that we predict might exert a hidden effect upon the regression model, specifically upon the dependent variable (i.e. cases per 100,000) and the primary independent variable of interest (i.e. whether the state implemented a mask use policy). These are all envisaged real-world phenomena that we do not have any data on, however for 3 of these, we can use proxy variables as provided in the dataset and external sources that we can utilize in bias estimation. We can predict the potential relationship between the omitted variables and the dependent and independent variables, and the direction of bias for all of our suggested omitted variables. We cannot estimate the size of the bias for the omitted variables that we do not have a proxy or direct information regarding, however.
...
Subsequently, $\tilde{\beta_1} - \hat{\beta_1}$ is used to calculate the actual omitted variable bias.

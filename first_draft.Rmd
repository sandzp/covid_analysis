...
It is also expected taht the more a state is able to provide tests, the higher the case rate will tend to be. This idea is supported 
...
To find the other control variables that best demonstrated the use of masks, we grouped similar variables together to find which variable or group of variables had the combination of highest correlation with case rate per 100k, and low colinearity with other variables. 

The following figure shows the correlation between case rate per 100k and various ethnic populations. The case rate per 100k variable has the highest correlation with the black population variable.

```{r warning=FALSE, message=FALSE}
ggpairs(data = df, columns = c(4,seq(14,20,2)))
```

Next, we look at case rate per 100k and the duration and days that people in the US had to shelter in place and businesses had to close (if at all). From the correlation plots below, the number of days that people had to shelter in place and the number of days a business had to close had the expected negative correlation with case rate per 100k. Another set of interesting variables are the beginning shelter date and business close date. The general correlation shows that the later a state started closures, the higher the case rate per 100k it tended to have.
...
From the figure below we can see that most variables have a low correlation (< 12%) with case rate per 100k. The exception may be the political leaning of the political party. The density plot and boxplot of case_rate ~ gov_party visually suggests that a relationship may exist.

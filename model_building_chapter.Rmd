...
Based on our causal model, mask use should lead to a decrease in the number of covid cases. The boxplot below suggests that this initial assumption holds to some degree.
...
Included in the dataset is the Google mobility metric regarding the change in activity of popular spots (retail, groceries, parks, etc.,). We find that the highest correlation of the mobility variables lies with the change in Transit Stations (mob_TS).
...
Surprisingly, the population density of a state has little to no correlation to the number of cases per capita in that state.
